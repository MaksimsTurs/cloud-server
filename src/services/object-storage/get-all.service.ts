...
export default async function getAll(user: User, id?: string): Promise<ObjectStorageServiceGetAllReturn> {
  ...
  if(!parent || parent.id !== parentId) {
    throw new CaughtError({
      server: {
        message: `Can not find parent directory with id ${parentId}`
      },
      client: HTTP_ERRORS.NOT_FOUND("Can not find directory!")
    });
  }

  return { items: items.slice(1, items.length), parent };
};
