...
function verifyToken<P = unknown>(token: string = "", secret: string): P & JwtPayload | undefined {
  ...
  return jsonwebtoken.verify(token, secret) as P & JwtPayload; 
};
...
export function verifyAccessToken<P = unknown>(token?: string): P & JwtPayload | undefined {
  return verifyToken(token, process.env.ACCESS_TOKEN_SECRET!);
};

export function verifyRefreshToken<P = unknown>(token?: string): P & JwtPayload | undefined {
